...
function checkIndexType() {
  /**
     ...
     */
  const $select_index_choice = $('#select_index_choice');
  /**
     * @var Object Dropdown to select the index type.
     */
  const $select_index_type = $('#select_index_type');
  /**
     * @var Object Table header for the size column.
     */
  const $size_header = $('#index_columns').find('thead tr th:nth-child(2)');
  /**
     * @var Object Inputs to specify the columns for the index.
     */
  const $column_inputs = $('select[name="index[columns][names][]"]');
  /**
     * @var Object Inputs to specify sizes for columns of the index.
     */
  const $size_inputs = $('input[name="index[columns][sub_parts][]"]');
  /**
     * @var Object Footer containg the controllers to add more columns
     */
  const $add_more = $('#index_frm').find('.add_more');
...
function PMA_removeColumnFromIndex(col_index) {
  ...
  let previous_index = $(`select[name="field_key[${col_index}]"]`)
    .attr('data-index');
  if (previous_index.length) {
    previous_index = previous_index.split(',');
    const source_array = PMA_getIndexArray(previous_index[0]);
    if (source_array === null) {
      return;
    }

    // Remove column from index array.
    const source_length = source_array[previous_index[1]].columns.length;
...
function PMA_addColumnToIndex(source_array, array_index, index_choice, col_index) {
  ...
  const index_name = $('input[name="index[Key_name]"]').val();
  const index_comment = $('input[name="index[Index_comment]"]').val();
  const key_block_size = $('input[name="index[Key_block_size]"]').val();
  const parser = $('input[name="index[Parser]"]').val();
  const index_type = $('select[name="index[Index_type]"]').val();
  const columns = [];
  $('#index_columns').find('tbody').find('tr').each(function () {
    // Get columns in particular order.
    const col_index = $(this).find('select[name="index[columns][names][]"]').val();
...
function PMA_getCompositeIndexList(source_array, col_index) {
  ...
  for (let i = 0; i < source_length; i++) {
    const sub_array_len = source_array[i].columns.length;
    const column_names = [];
    for (let j = 0; j < sub_array_len; j++) {
      column_names.push(
...
function PMA_showAddIndexDialog(source_array, array_index, target_columns, col_index, index) {
  ...
  for (let i = 0; i < target_columns.length; i++) {
    const column_name = $(`input[name="field_name[${target_columns[i]}]"]`).val();
    const column_type = $(`select[name="field_type[${target_columns[i]}]"]`).val().toLowerCase();
  ...
  button_options[PMA_messages.strCancel] = function () {
    if (col_index >= 0) {
      ...
      const $select_list = $(`select[name="field_key[${col_index}]"]`);
      if (!$select_list.attr('data-index').length) {
        $select_list.find('option[value*="none"]').attr('selected', 'selected');
      } else {
        const previous_index = $select_list.attr('data-index').split(',');
  ...
  $.post('tbl_indexes.php', post_data, (data) => {
...
function PMA_indexTypeSelectionDialog(source_array, index_choice, col_index) {
  const $single_column_radio = $(`${'<input type="radio" id="single_column" name="index_choice"'
        + ' checked="checked">'
        + '<label for="single_column">'}${PMA_messages.strCreateSingleColumnIndex}</label>`);
  const $composite_index_radio = $(`${'<input type="radio" id="composite_index"'
        + ' name="index_choice">'
        + '<label for="composite_index">'}${PMA_messages.strCreateCompositeIndex}</label>`);
  const $dialog_content = $('<fieldset id="advance_index_creator"></fieldset>');
  ...
  button_options[PMA_messages.strGo] = function () {
    ...
    if ($('#composite_index').is(':checked')) {
      ...
      const array_index = $('input[name="composite_with"]:checked').val();
      const source_length = source_array[array_index].columns.length;
  ...
  const $dialog = $('<div/>').append($dialog_content).dialog({
    minWidth: 525,
    minHeight: 200,
    modal: true,
    title: PMA_messages.strAddIndex,
 ...
 */
AJAX.registerTeardown('indexes.js', () => {
 ...
 * </ul>
 */
AJAX.registerOnload('indexes.js', () => {
  ...
  const $engine_selector = $('.create_table_form select[name=tbl_storage_engine]');
  ...
  $(document).on('click', '#save_index_frm', (event) => {
  ...
  $(document).on('click', '#preview_index_frm', (event) => {
    event.preventDefault();
    PMA_previewSQL($('#index_frm'));
  });

  $(document).on('change', '#select_index_choice', (event) => {
    event.preventDefault();
    checkIndexType();
    checkIndexName('index_frm');
  });
  ...
  $(document).on('click', 'a.drop_primary_key_index_anchor.ajax', function (event) {
    ...
    /**
         ...
         */
    const $curr_row = $anchor.parents('tr');
    /** @var    Number of columns in the key */
    const rows = $anchor.parents('td').attr('rowspan') || 1;
    /** @var    Rows that should be hidden */
    let $rows_to_hide = $curr_row;
    for (let i = 1, $last_row = $curr_row.next(); i < rows; i++, $last_row = $last_row.next()) {
    ...
    PMA_confirmPreviewSQL(question, $anchor.attr('href'), function (url) {
      ...
      $.post(url, params, (data) => {
        if (typeof data !== 'undefined' && data.success === true) {
          ...
          PMA_commonActions.refreshMain(false, () => {
  ...
  $('body').on('change', 'select[name*="field_key"]', function () {
    ...
    let col_index = /\d+/.exec($(this).attr('name'));
    col_index = col_index[0];
    // Choice of selected index.
    let index_choice = /[a-z]+/.exec($(this).val());
  ...
  $(document).on('click', '.show_index_dialog', function (e) {
    ...
    // Get index details.
    const previous_index = $(this).prev('select')
      .attr('data-index')
      .split(',');

    const index_choice = previous_index[0];
    const array_index = previous_index[1];

    const source_array = PMA_getIndexArray(index_choice);
    if (source_array !== null) {
      const source_length = source_array[array_index].columns.length;
