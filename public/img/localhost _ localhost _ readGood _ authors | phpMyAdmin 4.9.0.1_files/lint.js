...
(function (mod) {
  ...
  function makeMarker(labels, severity, multiple, tooltips) {
    const marker = document.createElement('div'); let
      inner = marker;
  ...
  function lintAsync(cm, getAnnotations, passOptions) {
    ...
    getAnnotations(cm.getValue(), (annotations, arg2) => {
      ...
      cm.operation(() => { updateLinting(cm, annotations); });
    }, passOptions, cm);
  }

  function startLinting(cm) {
    const state = cm.state.lint; const
      { options } = state;
    /*
    ...
    if (options.async || getAnnotations.async) {
      ...
      if (annotations.then) {
        annotations.then((issues) => {
          cm.operation(() => { updateLinting(cm, issues); });
        });
      } else cm.operation(() => { updateLinting(cm, annotations); });
    }
  }
  ...
  function onMouseOver(cm, e) {
    ...
    const box = target.getBoundingClientRect(); const x = (box.left + box.right) / 2; const
  ...
  CodeMirror.defineOption('lint', false, (cm, val, old) => {
    ...
    if (val) {
      const gutters = cm.getOption('gutters'); let
        hasLintGutter = false;
