...
var PMA_console = {
  ...
  initialize() {
    ...
    if (PMA_console.isInitialized === false) {
      ...
      $('#pma_console').find('.toolbar').children().mousedown((event) => {
        event.preventDefault();
        event.stopImmediatePropagation();
      });

      $('#pma_console').find('.button.clear').click(() => {
        PMA_consoleMessages.clear();
      });

      $('#pma_console').find('.button.history').click(() => {
        PMA_consoleMessages.showHistory();
      });

      $('#pma_console').find('.button.options').click(() => {
        PMA_console.showCard('#pma_console_options');
      });

      $('#pma_console').find('.button.debug').click(() => {
      ...
      $('#pma_console_options').find('input[type=checkbox]').change(() => {
        PMA_console.updateConfig();
      });

      $('#pma_console_options').find('.button.default').click(() => {
      ...
      $('#pma_console_options').find('input[name=enter_executes]').change(() => {
  ...
  collapse() {
    ...
    const pmaConsoleHeight = Math.max(92, PMA_console.config.Height);
  ...
  show(inputFocus) {
    ...
    let pmaConsoleHeight = Math.max(92, PMA_console.config.Height);
  ...
  isSelect(queryString) {
    const reg_exp = /^SELECT\s+/i;
...
var PMA_consoleInput = {
  ...
  initialize() {
    ...
    if (PMA_consoleInput._codemirror) {
      ...
      PMA_consoleInput._inputs.console.on('keydown', (instance, event) => {
      ...
      PMA_consoleInput._inputs.console = $('<textarea>').appendTo('#pma_console .console_query_input')
        .on('keydown', PMA_consoleInput._historyNavigate);
      if ($('#pma_bookmarks').length !== 0) {
        PMA_consoleInput._inputs.bookmark = $('<textarea>').appendTo('#pma_console .bookmark_add_input');
      }
    }
  ...
  _historyNavigate(event) {
    if (event.keyCode === 38 || event.keyCode === 40) {
      ...
      const editor = PMA_consoleInput._inputs.console;
      let cursorLine;
      let totalLine;
      if (PMA_consoleInput._codemirror) {
        ...
        const text = PMA_consoleInput.getText();
...
var PMA_consoleMessages = {
  ...
  appendQuery(queryData, state) {
    const targetMessage = PMA_consoleMessages.append(queryData.sql_query, 'query');
    if (!targetMessage) {
      return false;
    }
...
var PMA_consoleBookmarks = {
  ...
  initialize() {
    ...
    $('#pma_console').find('.button.bookmarks').click(() => {
      PMA_console.showCard('#pma_bookmarks');
    });
    $('#pma_bookmarks').find('.button.add').click(() => {
      PMA_console.showCard('#pma_bookmarks .card.add');
    });
    ...
    $('#pma_console').find('.button.refresh').click(() => {
      PMA_consoleBookmarks.refresh();
    });
  },
};

let PMA_consoleDebug;
PMA_consoleDebug = {
  _config: {
    groupQueries: false,
  ...
  initialize() {
    ...
    const orderBy = PMA_console.config.OrderBy;
    const order = PMA_console.config.Order;
    ...
    $('#debug_console').find('.button.group_queries').click(() => {
      $('#debug_console').addClass('grouped');
    ...
    $('#debug_console').find('.button.ungroup_queries').click(() => {
      $('#debug_console').addClass('ungrouped');
  ...
  showLog(debugInfo, url) {
    ...
    let totalUnique = 0;
    const uniqueArray = [];
    for (const hash in uniqueQueries) {
    ...
    function sortByTime(a, b) {
      const order = ((PMA_console.config.Order === 'asc') ? 1 : -1);
    ...
    function sortByCount(a, b) {
      const order = ((PMA_console.config.Oorder === 'asc') ? 1 : -1);
  ...
  refresh() {
    const last = this._lastDebugInfo;
