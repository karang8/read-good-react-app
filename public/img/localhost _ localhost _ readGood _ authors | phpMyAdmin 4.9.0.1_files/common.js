...
var PMA_commonParams = (function () {
  ...
  return {
    ...
    /**
         ...
         */
    getUrlQuery() {
      const common = this.get('common_query');
      let separator = '?';
...
PMA_DROP_IMPORT = {
  ...
  /**
     ...
     * @return void
     */
  _sendFileToServer(formData, hash) {
    const uploadURL = './import.php'; // Upload URL
    const extraData = {};
    const jqXHR = $.ajax({
      xhr() {
  ...
  _dragleave(event) {
    ...
    event.stopPropagation();
    event.preventDefault();
    const $pma_drop_handler = $('.pma_drop_handler');
  ...
  /**
     ...
     * @return void
     */
  _importFinished(hash, aborted, status) {
    $(`.pma_sql_import_status div li[data-hash="${hash}"]`)
  ...
  _drop(event) {
    ...
    const dbname = PMA_commonParams.get('db');
    const server = PMA_commonParams.get('server');
    ...
    if (dbname !== '') {
      ...
      for (let i = 0; i < files.length; i++) {
        const ext = (PMA_DROP_IMPORT._getExtension(files[i].name));
        const hash = AJAX.hash(++PMA_DROP_IMPORT.uploadCount);

        const $pma_sql_import_status_div = $('.pma_sql_import_status div');
...
$(document).on('click', '.pma_sql_import_status h2 .close', () => {
  $('.pma_sql_import_status').fadeOut(() => {
