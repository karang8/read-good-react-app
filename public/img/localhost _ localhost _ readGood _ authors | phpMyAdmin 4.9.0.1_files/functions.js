 ...
 */
let sql_box_locked = false;

/**
 * @var array holds elements which content should only selected once
 */
const only_once_elements = [];
  ...
  const defaultOptions = {
    ...
    beforeShow(input, inst) {
      ...
      setTimeout(() => {
        ...
        const tooltip = $this_element.tooltip('instance');
...
function addDateTimePicker() {
  if ($.timepicker !== undefined) {
    $('input.timefield, input.datefield, input.datetimefield').each(function () {
      const decimals = $(this).parent().attr('data-decimals');
...
function PMA_hideShowDefaultValue($default_type) {
  if ($default_type.val() === 'USER_DEFINED') {
    ...
    if ($default_type.val() === 'NULL') {
      const $null_checkbox = $default_type.closest('tr').find('.allow_null');
...
function suggestPassword(passwd_form) {
  ...
  const pwchars = 'abcdefghijklmnopqrstuvwxyz0123456789ABCDEFGHIJKLMNOPQRSTUVWYXZ';
  const passwordlength = 16; // do we want that to be dynamic?  no, keep it simple :)
  const passwd = passwd_form.generated_pw;
...
function PMA_current_version(data) {
  if (data && data.version && data.date) {
    const current = parseVersionString($('span.version').text());
    const latest = parseVersionString(data.version);
    const url = `https://www.phpmyadmin.net/files/${escapeHtml(encodeURIComponent(data.version))}/`;
    let version_information_message = document.createElement('span');
    version_information_message.className = 'latest';
    const version_information_message_link = document.createElement('a');
    ...
    const prefix_message = document.createTextNode(`${PMA_messages.strLatestAvailable} `);
    ...
    if (latest > current) {
      const message = PMA_sprintf(
        PMA_messages.strNewerVersion,
        escapeHtml(data.version),
        escapeHtml(data.date),
      );
      let htmlClass = 'notice';
      if (Math.floor(latest / 10000) === Math.floor(current / 10000)) {
        /* Security update */
        htmlClass = 'error';
      }
      $('#newer_version_notice').remove();
      const maincontainer_div = document.createElement('div');
      maincontainer_div.id = 'newer_version_notice';
      maincontainer_div.className = htmlClass;
      const maincontainer_div_link = document.createElement('a');
    ...
    const version_info = $('#li_pma_version').contents().get(2);
...
function displayPasswordGenerateButton() {
  const generatePwdRow = $('<tr />').addClass('vmiddle');
  const titleCell = $('<td />').html(PMA_messages.strGeneratePassword).appendTo(generatePwdRow);
  const pwdCell = $('<td />').appendTo(generatePwdRow);
  const pwdButton = $('<input />')
  ...
  const generatePwdDiv = $('<div />').addClass('item');
  const titleLabel = $('<label />').attr({ for: 'button_generate_password' })
    .html(`${PMA_messages.strGeneratePassword}:`)
    .appendTo(generatePwdDiv);
  const optionsSpan = $('<span/>').addClass('options')
    .appendTo(generatePwdDiv);
...
function confirmLink(theLink, theSqlQuery) {
  ...
  const is_confirmed = confirm(PMA_sprintf(PMA_messages.strDoYouReally, theSqlQuery));
  if (is_confirmed) {
...
function confirmQuery(theForm1, sqlQuery1) {
  ...
  const do_confirm_re_0 = new RegExp('^\\s*DROP\\s+(IF EXISTS\\s+)?(TABLE|PROCEDURE)\\s', 'i');
  const do_confirm_re_1 = new RegExp('^\\s*ALTER\\s+TABLE\\s+((`[^`]+`)|([A-Za-z0-9_$]+))\\s+DROP\\s', 'i');
  const do_confirm_re_2 = new RegExp('^\\s*DELETE\\s+FROM\\s', 'i');
  const do_confirm_re_3 = new RegExp('^\\s*TRUNCATE\\s', 'i');
  ...
  if (do_confirm_re_0.test(sqlQuery1)
    ...
    const is_confirmed = confirm(PMA_sprintf(PMA_messages.strDoYouReally, message));
 ...
 */
let last_click_checked = false;

/**
 * Zero-based index of last clicked row.
 * Used to handle the shift + click event in the code above.
 */
let last_clicked_row = -1;

/**
 * Zero-based index of last shift clicked row.
 */
let last_shift_clicked_row = -1;

let _idleSecondsCounter = 0;
let IncInterval;
let updateTimeout;
AJAX.registerTeardown('functions.js', () => {
  clearTimeout(updateTimeout);
  clearInterval(IncInterval);
  $(document).off('mousemove');
});

AJAX.registerOnload('functions.js', () => {
  ...
  if (PMA_commonParams.get('logged_in')) {
    ...
    let interval = (session_timeout - 5) * 1000;
...
AJAX.registerOnload('functions.js', () => {
  ...
  $(document).on('click', 'input:checkbox.checkall', function (e) {
    ...
    if (!e.shiftKey || last_clicked_row === -1) {
      ...
      const curr_row = $table.find('tr:not(.noclick)').index($tr);
...
function PMA_handleSimulateQueryButton() {
  const update_re = new RegExp('^\\s*UPDATE\\s+((`[^`]+`)|([A-Za-z0-9_$]+))\\s+SET\\s', 'i');
  const delete_re = new RegExp('^\\s*DELETE\\s+FROM\\s', 'i');
  let query = '';
...
function insertQuery(queryType) {
  ...
  const table = document.sqlform.table.value;
...
function updateQueryParameters() {
  if ($('#parameterized').is(':checked')) {
    const query = codemirror_editor ? codemirror_editor.getValue() : $('#sqlquery').val();

    const allParameters = query.match(/:[a-zA-Z0-9_]+/g);
    const parameters = [];
    // get unique parameters
    if (allParameters) {
      $.each(allParameters, (i, parameter) => {
...
function refreshLayout() {
  const $elm = $('#pdflayout');
  const orientation = $('#orientation_opt').val();
  let paper = 'A4';
  const $paperOpt = $('#paper_opt');
...
function TableDragInit() {
  $('.pdflayout_table').each(function () {
    ...
    const x = $(`#c_table_${number}_x`).val();
    const y = $(`#c_table_${number}_y`).val();
    $this.css('left', `${x}px`);
    $this.css('top', `${y}px`);
...
$(() => {
  ...
  $(document).on('click', '#toggle-dragdrop', () => {
    const $elm = $('#pdflayout');
  ...
  /* Reset scratchboard */
  $(document).on('click', '#reset-dragdrop', () => {
    resetDrag();
  });
});
...
function getJSConfirmCommonParam(elem, params) {
  const $elem = $(elem);
  const sep = PMA_commonParams.get('arg_separator');
  if (params) {
...
AJAX.registerOnload('functions.js', () => {
  ...
  $(document).on('click', 'a.inline_edit_sql', function () {
    ...
    const sql_query = $form.find('input[name=\'sql_query\']').val().trim();
    const $inner_sql = $(this).parent().prev().find('code.sql');
    const old_text = $inner_sql.html();

    let new_content = `<textarea name="sql_query_edit" id="sql_query_edit">${escapeHtml(sql_query)}</textarea>\n`;
    new_content += getForeignKeyCheckboxLoader();
    new_content += `<input type="submit" id="sql_query_edit_save" class="button btnSave" value="${PMA_messages.strGo}"/>\n`;
    new_content += `<input type="button" id="sql_query_edit_discard" class="button btnDiscard" value="${PMA_messages.strCancel}"/>\n`;
    let $editor_area = $('div#inline_editor');
  ...
  $(document).on('click', 'input#sql_query_edit_save', function () {
    ...
    const fk_check = $(this).parent().find('#fk_checks').is(':checked');

    const $form = $('a.inline_edit_sql').prev('form');
    const $fake_form = $('<form>', { action: 'import.php', method: 'post' })
  ...
  $(document).on('click', 'input#sql_query_edit_discard', () => {
    const $divEditor = $('div#inline_editor_outer');
    $divEditor.siblings('code.sql').show();
    $divEditor.remove();
  });

  $(document).on('click', 'input.sqlbutton', (evt) => {
...
function codemirrorAutocompleteOnInputRead(instance) {
  if (!sql_autocomplete_in_progress
    ...
    if (!sql_autocomplete) {
      ...
      const href = 'db_sql_autocomplete.php';
      const params = {
        ajax_request: true,
        server: PMA_commonParams.get('server'),
        db: PMA_commonParams.get('db'),
              ...
              if (tables.hasOwnProperty(table)) {
                ...
                for (const column in columns) {
                  if (columns.hasOwnProperty(column)) {
                    let displayText = columns[column].Type;
...
function PMA_doc_keyword(idx, elm) {
  ...
  if ($next) {
    const next_keyword = $next.text().toUpperCase();
    const full = `${keyword} ${next_keyword}`;

    const $next2 = $next.next('.cm-keyword');
    if ($next2) {
      const next2_keyword = $next2.text().toUpperCase();
      const full2 = `${full} ${next2_keyword}`;
...
function PMA_previewSQL($form) {
  ...
  const form_data = `${$form.serialize()
    ...
    sep}preview_sql=1${
    sep}ajax_request=1`;
  const $msgbox = PMA_ajaxShowMessage();
  $.ajax({
    type: 'POST',
    url: form_url,
    data: form_data,
    success(response) {
      ...
      if (response.success) {
        ...
        const $response_dialog = $dialog_content.dialog({
          minWidth: 550,
          maxHeight: 400,
          modal: true,
...
function PMA_confirmPreviewSQL(sql_data, url, callback) {
  const $dialog_content = $(`<div class="preview_sql"><code class="sql"><pre>${
    sql_data
  }</pre></code></div>`);
  const button_options = [
    {
...
function PMA_showNoticeForEnum(selectElement) {
  let enum_notice_id = selectElement.attr('id').split('_')[1];
...
function PMA_createProfilingChart(target, data) {
  ...
  const chart = factory.createChart(ChartType.PIE, target);
...
function PMA_SQLPrettyPrint(string) {
  ...
  const mode = CodeMirror.getMode({}, 'text/x-mysql');
  const stream = new CodeMirror.StringStream(string);
  const state = mode.startState();
  let token;
  const tokens = [];
  let output = '';
  const tabs = function (cnt) {
    let ret = '';
  ...
  const spaceExceptionsBefore = {
    ';': true, ',': true, '.': true, '(': true,
  };
  // don't put spaces after these tokens
  const spaceExceptionsAfter = { '.': true };
  ...
  let currentStatement = tokens[0][1];
  ...
  let statementPart;
  let lastStatementPart = statements[currentStatement][0];
...
jQuery.fn.PMA_confirm = function (question, url, callbackFn, openCallback) {
  const confirmState = PMA_commonParams.get('confirm');
  if (!confirmState) {
...
AJAX.registerOnload('functions.js', () => {
  ...
  $(document).on('submit', 'form.create_table_form.ajax', function (event) {
    ...
    if (checkTableEditForm($form[0], $form.find('input[name=orig_num_fields]').val())) {
      ...
      if (PMA_checkReservedWordColumns($form)) {
        ...
        $.post($form.attr('action'), `${$form.serialize() + PMA_commonParams.get('arg_separator')}do_save_data=1`, (data) => {
          if (typeof data !== 'undefined' && data.success === true) {
            $('#properties_message')
              .removeClass('error')
              .html('');
            ...
            const $createTableDialog = $('#create_table_dialog');
            ...
            /**
                         ...
                         */
            const tables_table = $('#tablesForm').find('tbody').not('#tbl_summary_row');
            ...
            if (tables_table.length === 0) {
              ...
              /**
                             ...
                             */
              const curr_last_row = $(tables_table).find('tr:last');
              /**
                             * @var curr_last_row_index_string   String containing the index of {@link curr_last_row}
                             */
              const curr_last_row_index_string = $(curr_last_row).find('input:checkbox').attr('id').match(/\d+/)[0];
              /**
                             * @var curr_last_row_index Index of {@link curr_last_row}
                             */
              const curr_last_row_index = parseFloat(curr_last_row_index_string);
              /**
                             * @var new_last_row_index   Index of the new row to be appended to {@link tables_table}
                             */
              const new_last_row_index = curr_last_row_index + 1;
              /**
                             * @var new_last_row_id String containing the id of the row to be appended to {@link tables_table}
                             */
              const new_last_row_id = `checkbox_tbl_${new_last_row_index}`;
            ...
            const argsep = PMA_commonParams.get('arg_separator');
            let params_12 = `ajax_request=true${argsep}ajax_page_request=true`;
  ...
  function submitChangesInCreateTableForm(actionParam) {
    ...
    const $msgbox = PMA_ajaxShowMessage(PMA_messages.strProcessingRequest);
    PMA_prepareForAjaxRequest($form);

    // User wants to add more fields to the table
    $.post($form.attr('action'), `${$form.serialize()}&${actionParam}`, (data) => {
  ...
  /**
     ...
     */
  $(document).on('click', 'form.create_table_form.ajax input[name=submit_num_fields]', (event) => {
    event.preventDefault();
...
function PMA_checkPassword($the_form) {
  ...
  const $pred = $the_form.find('#select_pred_password');
  if ($pred.length && ($pred.val() === 'none' || $pred.val() === 'keep')) {
    return true;
  }


  const $password = $the_form.find('input[name=pma_pw]');
  const $password_repeat = $the_form.find('input[name=pma_pw2]');
  let alert_msg = false;
...
AJAX.registerOnload('functions.js', () => {
  ...
  $(document).on('change', '#pma_hostname', () => {
  ...
  $(document).on('change', '#pma_username', () => {
  ...
  $(document).on('change', '#text_pma_pw,#text_pma_pw2', () => {
  ...
  $(document).on('click', '#change_password_anchor.ajax', function (event) {
    ...
    button_options[PMA_messages.strGo] = function () {
      ...
      /**
             ...
             */
      const $the_form = $('#change_password_form');
      ...
      $.post($the_form.attr('action'), `${$the_form.serialize() + PMA_commonParams.get('arg_separator')}change_pw=${this_value}`, (data) => {
        if (typeof data === 'undefined' || data.success !== true) {
    ...
    $.get($(this).attr('href'), { ajax_request: true }, (data) => {
      if (typeof data === 'undefined' || !data.success) {
        PMA_ajaxShowMessage(data.error, false);
        return;
      }
...
function PMA_hideShowConnection($engine_selector) {
  ...
  const index = $connection.parent('td').index() + 1;
  const $labelTh = $connection.parents('tr').prev('tr').children(`th:nth-child(${index})`);
...
function PMA_validateDefaultValue($null_checkbox) {
  if (!$null_checkbox.prop('checked')) {
    const $default = $null_checkbox.closest('tr').find('.default_type');
    if ($default.val() === 'NULL') {
      $default.val('NONE');
    }
  }
...
function autoPopulate(input_id, offset) {
  const db = PMA_commonParams.get('db');
  const table = PMA_commonParams.get('table');
  input_id = input_id.substring(0, input_id.length - 1);
  $(`#${input_id}1`).val(central_column_list[`${db}_${table}`][offset].col_name);
  const col_type = central_column_list[`${db}_${table}`][offset].col_type.toUpperCase();
  $(`#${input_id}2`).val(col_type);
  const $input3 = $(`#${input_id}3`);
  ...
  const col_default = central_column_list[`${db}_${table}`][offset].col_default.toUpperCase();
  const $input4 = $(`#${input_id}4`);
...
AJAX.registerOnload('functions.js', () => {
  $(document).on('click', 'a.open_enum_editor', function () {
    ...
    const colname = $(this).closest('tr').find('input:first').val();
    let title;
    let i;
    ...
    const drop_icon = PMA_getImage('b_drop');
    ...
    /**
         ...
         */
    const dialog = `${'<div id=\'enum_editor\'>'
                   + '<fieldset>'
                    + '<legend>'}${title}</legend>`
    ...
    buttonOptions[PMA_messages.strGo] = function () {
      ...
      const value_array = [];
      $(this).find('.values input').each((index, elm) => {
        const val = elm.value.replace(/\\/g, '\\\\').replace(/'/g, '\'\'');
        value_array.push(`'${val}'`);
      });
      // get the Length/Values text field where this value belongs
      const values_id = $(this).find('input[type=\'hidden\']').val();
  ...
  $(document).on('click', 'a.central_columns_dialog', function (e) {
    const href = 'db_central_columns.php';
    const db = PMA_commonParams.get('db');
    const table = PMA_commonParams.get('table');
    ...
    let i = 0;
    const list_size = central_column_list[`${db}_${table}`].length;
    let min = (list_size <= maxRows) ? list_size : maxRows;
    for (i = 0; i < min; i++) {
    ...
    let result_pointer = i;
    let search_in = `<input type="text" class="filter_rows" placeholder="${PMA_messages.searchList}">`;
    if (fields === '') {
    ...
    const central_columns_dialog = `${'<div style=\'max-height:400px\'>'
            + '<fieldset>'}${
      search_in
  ...
  $(document).on('click', 'input.add_value', (e) => {
    e.preventDefault();
    let num_new_rows = $enum_editor_dialog.find('div.slider').slider('value');
    while (num_new_rows--) {
...
function checkIndexName(form_id) {
  ...
  const $the_idx_name = $('#input_index_name');
  const $the_idx_choice = $('#select_index_choice');
...
function indexEditorDialog(url, title, callback_success, callback_failure) {
  ...
  const $editIndexDialog = $('#edit_index_dialog');
  if ($editIndexDialog.length !== 0) {
    $editIndexDialog.remove();
  }
  const $div = $('<div id="edit_index_dialog"></div>');
  ...
  button_options[PMA_messages.strGo] = function () {
    ...
    $.post($form.attr('action'), `${$form.serialize() + PMA_commonParams.get('arg_separator')}do_save_data=1`, (data) => {
      ...
      if (typeof data !== 'undefined' && data.success === true) {
        ...
        const $temp_div = $('<div id=\'temp_div\'><div>').append(data.error);
        let $error;
 ...
 */
const toggleButton = function ($obj) {
  ...
  /**
     ...
     */
  const w = parseInt(($('img', $obj).height() / 16) * 22, 10);
  ...
  const offset = parseInt(((imgw - tblw) / 2), 10);
  ...
  const btnw = $('table td:nth-child(2)', $obj).outerWidth();
  ...
  $('div.container', $obj).click(function () {
    ...
    $.post(parts[0], `${parts[1]}&ajax_request=true`, (data) => {
      if (typeof data !== 'undefined' && data.success === true) {
        PMA_ajaxRemoveMessage($msg);
        $container
          .removeClass(removeClass)
          .addClass(addClass)
          .animate({ left: `${operator + move}px` }, () => {
...
AJAX.registerOnload('functions.js', () => {
  ...
  const $updateRecentTables = $('#update_recent_tables');
...
function submitFormLink($link) {
  if ($link.attr('href').indexOf('=') !== -1) {
    const data = $link.attr('href').substr($link.attr('href').indexOf('#') + 1).split('=', 2);
...
function PMA_init_slider() {
  $('div.pma_auto_slider').each(function () {
    ...
    const $wrapper = $('<div>', { class: 'slide-wrapper' });
    ...
    $('<a>', { href: `#${this.id}`, class: 'ajax' })
      ...
      .insertBefore($this)
      .click(() => {
        const $wrapper = $this.closest('.slide-wrapper');
        const visible = $this.is(':visible');
        if (!visible) {
          $wrapper.show();
        }
        $this[visible ? 'hide' : 'show']('blind', () => {
...
AJAX.registerOnload('functions.js', () => {
  /**
     * Theme selector.
     */
  $(document).on('click', 'a.themeselect', (e) => {
    window.open(
      e.target,
      'themes',
  ...
  $.post($this.attr('href'), params, (data) => {
    if (typeof data !== 'undefined' && data.success === true) {
      ...
      buttonOptions[PMA_messages.strGo] = function () {
        ...
        $.post('view_create.php', $('#createViewDialog').find('form').serialize(), (data) => {
          PMA_ajaxRemoveMessage($msg);
          if (typeof data !== 'undefined' && data.success === true) {
      ...
      const $dialog = $('<div/>').attr('id', 'createViewDialog').append(data.message).dialog({
        width: 600,
        minWidth: 400,
        modal: true,
        buttons: buttonOptions,
 ...
 */
$(() => {
  $(document).on('click', '#serverinfo, #goto_pagetop', (event) => {
    event.preventDefault();
    $('html, body').animate({ scrollTop: 0 }, 'fast');
  });
});

const checkboxes_sel = 'input.checkall:checkbox:enabled';
/**
 * Watches checkboxes in a form to set the checkall box accordingly
 */
const checkboxes_changed = function () {
  const $form = $(this.form);
  // total number of checkboxes in current form
  const total_boxes = $form.find(checkboxes_sel).length;
  // number of checkboxes checked in current form
  const checked_boxes = $form.find(`${checkboxes_sel}:checked`).length;
...
$(document).on('click', '.checkall-filter', function () {
  ...
  $this.parents('form').find(checkboxes_sel).filter(selector).prop('checked', true)
    .trigger('change')
    .parents('tr')
 ...
 */
const sub_checkboxes_changed = function () {
...
$(document).on('keyup', '#filterText', function () {
  const filterInput = $(this).val().toUpperCase();
  let count = 0;
...
function formatBytes(bytes, subdecimals, pointchar) {
  ...
  const factor = Math.pow(10, subdecimals);
...
AJAX.registerOnload('functions.js', () => {
  ...
  const $https_warning = $('#js-https-mismatch');
...
function checkNumberOfFields() {
  ...
  $('form').each(function () {
    ...
    if (nbInputs > maxInputVars) {
      const warning = PMA_sprintf(PMA_messages.strTooManyInputs, maxInputVars);
