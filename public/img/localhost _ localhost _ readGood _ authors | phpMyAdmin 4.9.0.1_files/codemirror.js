...
(function (global, factory) {
  ...
  const Delayed = function () { this.id = null; };
  ...
  function clipToLen(pos, linelen) {
    const { ch } = pos;
    if (ch == null || ch > linelen) { return Pos(pos.line, linelen); } if (ch < 0) { return Pos(pos.line, 0); } return pos;
  }
    ...
    if (old) {
      for (let i = 0; i < old.length; ++i) {
        ...
        if (startsBefore || span.from == startCh && marker.type == 'bookmark' && (!isInsert || !span.marker.insertLeft)) {
          const endsAfter = span.to == null || (marker.inclusiveRight ? span.to >= startCh : span.to > startCh); (nw || (nw = [])).push(new MarkedSpan(marker, span.from, endsAfter ? null : span.to));
        }
      }
    }
    return nw;
    ...
    if (old) {
      for (let i = 0; i < old.length; ++i) {
        ...
        if (endsAfter || span.from == endCh && marker.type == 'bookmark' && (!isInsert || span.marker.insertLeft)) {
          const startsBefore = span.from == null || (marker.inclusiveLeft ? span.from <= endCh : span.from < endCh); (nw || (nw = [])).push(new MarkedSpan(marker, startsBefore ? null : span.from - endCh,
  ...
  // Character types for codepoints 0 to 0xff
    ...
    function charType(code) {
      if (code <= 0xf7) { return lowTypes.charAt(code); } if (code >= 0x590 && code <= 0x5f4) { return 'R'; } if (code >= 0x600 && code <= 0x6f9) { return arabicTypes.charAt(code - 0x600); } if (code >= 0x6ee && code <= 0x8ac) { return 'r'; } if (code >= 0x2000 && code <= 0x200b) { return 'w'; } if (code == 0x200c) { return 'b'; } return 'L';
    }
  ...
  function resolveMode(spec) {
    ...
    if (typeof spec === 'string') { return { name: spec }; } return spec || { name: 'null' };
  }
  ...
  function insertLineContent(line, builder, styles) {
    ...
    const len = allText.length; let pos = 0; let i = 1; let text = ''; let style; let
      css;
    let nextChange = 0; let spanStyle; let spanEndStyle; let spanStartStyle; let title; let
      collapsed;
    for (;;) {
    ...
    if (cm.options.lineNumbers || markers) {
      ...
      if (markers) {
        for (let k = 0; k < cm.options.gutters.length; ++k) {
          const id = cm.options.gutters[k]; const
  ...
  function offsetPos(pos, old, nw) {
    if (pos.line == old.line) { return Pos(nw.line, pos.ch - old.ch + nw.ch); } return Pos(nw.line + (pos.line - old.line), pos.ch);
  }
  ...
  function filterSelectionChange(doc, sel, options) {
    ...
    if (obj.ranges != sel.ranges) { return normalizeSelection(obj.ranges, obj.ranges.length - 1); } return sel;
  }
  ...
  function movePos(doc, pos, dir, line) {
    if (dir < 0 && pos.ch == 0) {
      if (pos.line > doc.first) { return clipPos(doc, Pos(pos.line - 1)); } return null;
    } if (dir > 0 && pos.ch == (line || getLine(doc, pos.line)).text.length) {
      if (pos.line < doc.first + doc.size - 1) { return Pos(pos.line + 1, 0); } return null;
    }
  ...
  TextMarker.prototype.detachLine = function (line) {
    ...
    if (!this.lines.length && this.doc.cm) {
      const op = this.doc.cm.curOp; (op.maybeHiddenMarkers || (op.maybeHiddenMarkers = [])).push(this);
    }
  };
  ...
  function leftButtonStartDrag(cm, event, pos, behavior) {
    ...
    var dragEnd = operation(cm, (e) => {
      ...
      if (!moved) {
        ...
        if (webkit || ie && ie_version == 9) { setTimeout(() => { document.body.focus(); display.input.focus(); }, 20); } else { display.input.focus(); }
      }
    });
  ...
  function CodeMirror$1(place, options) {
    ...
    const input = new CodeMirror$1.inputStyles[options.inputStyle](this);
  ...
  const addEditorMethods = function (CodeMirror) {
    const { optionHandlers } = CodeMirror;
  ...
  ContentEditableInput.prototype.showPrimarySelection = function () {
    const sel = window.getSelection(); const { cm } = this; const
  ...
  function locateNodeInLineView(lineView, node, offset) {
    ...
    for (let after = topNode.nextSibling, dist = textNode ? textNode.nodeValue.length - offset : 0; after; after = after.nextSibling) {
      ...
      if (found) { return badPos(Pos(found.line, found.ch - dist), bad); } dist += after.textContent.length;
    }
    for (let before = topNode.previousSibling, dist$1 = offset; before; before = before.previousSibling) {
      found = find(before, before.firstChild, -1);
      if (found) { return badPos(Pos(found.line, found.ch + dist$1), bad); } dist$1 += before.textContent.length;
    }
  }
  ...
  function fromTextArea(textarea, options) {
    ...
    var cm = CodeMirror$1(node => textarea.parentNode.insertBefore(node, textarea.nextSibling),
      options);
    return cm;
  }
  ...
  CodeMirror$1.defineMode('null', () => ({ token(stream) { return stream.skipToEnd(); } }));
