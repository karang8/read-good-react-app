...
(function (factory) {
  ...
  const oldIsNumeric = jQuery.isNumeric;
  ...
  jQuery.find = function (selector) {
    ...
    if (typeof selector === 'string' && rattrHashTest.test(selector)) {
      ...
      try {
        ...
        selector = selector.replace(rattrHashGlob, (_, attr, op, value) => `[${attr}${op}"${value}"]`);
  ...
  jQuery.isNumeric = function (val) {
    ...
    const newValue = oldIsNumeric(val);
    const oldValue = isNumeric2(val);
  ...
  jQuery.fn.removeAttr = function (name) {
    ...
    jQuery.each(name.match(rmatchNonSpace), (i, attr) => {
  ...
  if (jQuery.swap) {
    jQuery.each(['height', 'width', 'reliableMarginRight'], (_, name) => {
