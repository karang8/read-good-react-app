...
function navFilterStateRestore() {
  if (isStorageSupported('sessionStorage')
    ...
    $tableFilters.each(function () {
      ...
      const filterName = $(this).siblings('input[name=aPath]').val();
...
function loadChildNodes(isNode, $expandElem, callback) {
  ...
  const url = $('#pma_navigation').find('a.navigation_url').attr('href');
  $.get(url, params, (data) => {
...
function traverseNavigationForPaths() {
  ...
  $('#pma_navigation_tree').find('a.expander:visible').each(function () {
    if ($(this).find('img').is('.ic_b_minus')
      ...
      let pos2_name = $(this).find('span.pos2_name').text();
      if (!pos2_name) {
        pos2_name = $(this)
          .parent()
          .parent()
      ...
      let pos2_value = $(this).find('span.pos2_value').text();
      if (!pos2_value) {
        pos2_value = $(this)
          .parent()
          .parent()
...
$(() => {
  ...
  $(document).on('submit', '.fast_filter', (event) => {
  ...
  /**
     ...
     */
  $(document).on('click', '#pma_navigation_reload', (event) => {
    event.preventDefault();

    // Find the loading symbol and show it
    const $icon_throbber_src = $('#pma_navigation').find('.throbber');
  ...
  /**
     ...
     */
  $(document).on('click', '#pma_navigation_collapse', (event) => {
    event.preventDefault();
  ...
  /**
     ...
     */
  $(document).on('mouseenter', '#pma_navigation_sync', (event) => {
    event.preventDefault();
  ...
  /**
     ...
     */
  $(document).on('mouseout', '#pma_navigation_sync', (event) => {
    event.preventDefault();
    const synced = $('#pma_navigation_tree').hasClass('synced');
    const $img = $('#pma_navigation_sync').children('img');
    if (synced) {
  ...
  /**
     ...
     */
  $(document).on('click', '#pma_navigation_sync', (event) => {
    event.preventDefault();
    const synced = $('#pma_navigation_tree').hasClass('synced');
    const $img = $('#pma_navigation_sync').children('img');
    if (synced) {
  ...
  $(document).on('click', '#pma_navigation_tree li.new_index a.ajax', function (event) {
    event.preventDefault();
    const url = `${$(this).attr('href').substr(
      $(this).attr('href').indexOf('?') + 1,
    ) + PMA_commonParams.get('arg_separator')}ajax_request=true`;
    const title = PMA_messages.strAddIndex;
  ...
  $(document).on('click', 'li.index a.ajax', function (event) {
    ...
    const title = PMA_messages.strEditIndex;
    indexEditorDialog(url, title);
  });
  ...
  if (isStorageSupported('sessionStorage')) {
    ...
    $(document).on('submit', 'form.config-form', (event) => {
      storage.removeItem('navTreePaths');
    });
...
function expandTreeNode($expandElem, callback) {
  ...
  if ($expandElem.hasClass('loaded')) {
    ...
    loadChildNodes(true, $expandElem, (data) => {
      if (typeof data !== 'undefined' && data.success === true) {
...
function PMA_showCurrentNavigation() {
  ...
  if (db) {
    ...
    if ($('#navi_db_select').length
      ...
      if ($('#pma_navigation_tree_content').find('span.loaded_db:first').text()
        ...
        loadChildNodes(false, $('option:selected', $('#navi_db_select')), (data) => {
  ...
  function loadAndHighlightTableOrView($dbItem, itemName) {
    const $container = $dbItem.children('div.list_container');
    let $expander;
    let $whichItem = isItemInContainer($container, itemName, 'li.table, li.view');
    ...
    if ($whichItem) {
      ...
      const $sub_containers = $dbItem.find('.subContainer');
  ...
  function loadAndShowTableOrView($expander, $relatedContainer, itemName) {
    loadChildNodes(true, $expander, (data) => {
...
function PMA_navigationTreePagination($this) {
  ...
  const searchClause = PMA_fastFilter.getSearchClause();
  if (searchClause) {
  ...
  if (isDbSelector) {
    ...
    const searchClause2 = PMA_fastFilter.getSearchClause2($this);
    if (searchClause2) {
...
var ResizeHandler = function () {
  ...
  this.setWidth = function (pos) {
    ...
    const resizer_width = $resizer.width();
  ...
  this.collapse = function (event) {
    event.preventDefault();
    let { panel_width } = event.data.resize_handler;
  ...
  this.treeResize = function (event) {
    ...
    const $nav_tree = $('#pma_navigation_tree');
    const $nav_header = $('#pma_navigation_header');
    const $nav_tree_content = $('#pma_navigation_tree_content');
...
var PMA_fastFilter = {
  ...
  getSearchClause() {
    let retval = '';
    const $input = $('#pma_navigation_tree')
  ...
  events: {
    ...
    keyup(event) {
      ...
      var item_filter = function ($curr) {
        ...
        $curr.children('ul').children('li').children('a').not('.container')
          .each(function () {
    ...
    clear(event) {
      ...
      const value = $(this).prev()[0].defaultValue;
...
PMA_fastFilter.filter.prototype.request = function () {
  ...
  if (self.$this.find('> ul > li > form.fast_filter:first input[name=searchClause]').length === 0) {
    const $input = $('#pma_navigation_tree').find('li.fast_filter.db_fast_filter input.searchClause');
    if ($input.length && $input.val() !== $input[0].defaultValue) {
