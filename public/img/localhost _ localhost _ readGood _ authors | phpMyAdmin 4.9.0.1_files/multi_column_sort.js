...
function removeColumnFromMultiSort(target, parent) {
  const URL = captureURL(target);
  const begin = target.indexOf('ORDER+BY') + 8;
  const end = target.indexOf(`${PMA_commonParams.get('arg_separator')}session_max_rows`);
  // get the names of the columns involved
  const between_part = target.substr(begin, end - begin);
  const columns = between_part.split('%2C+');
  // If the given column is not part of the order clause exit from this function
  const index = parent.find('small').length ? parent.find('small').text() : '';
  if (index === '') {
    return '';
  }
  ...
  const middle_part = columns.join('%2C+');
  params = URL.head + middle_part + URL.tail;
  return params;
}

AJAX.registerOnload('keyhandler.js', () => {
...
AJAX.registerTeardown('keyhandler.js', () => {
