...
function loadJSAndGISEditor(value, field, type, input_name) {
  ...
  const smallScripts = ['js/vendor/jquery/jquery.svg.js',
...
function openGISEditor() {
  ...
  const $gis_editor = $('#gis_editor');
  const $backgrouond = $('#popup_background');

  $gis_editor.css({
    top: popupOffsetTop, left: popupOffsetLeft, width: popupWidth, height: popupHeight,
  });
...
function insertDataAndClose() {
  ...
  const input_name = $form.find('input[name=\'input_name\']').val();

  const argsep = PMA_commonParams.get('arg_separator');
  $.post('gis_data_editor.php', `${$form.serialize() + argsep}generate=true${argsep}ajax_request=true`, (data) => {
    if (typeof data !== 'undefined' && data.success === true) {
 ...
 */
AJAX.registerTeardown('gis_data_editor.js', () => {
...
AJAX.registerOnload('gis_data_editor.js', () => {
  /**
     * Prepares and insert the GIS data to the input field on clicking 'copy'.
     */
  $(document).on('click', '#gis_editor input[name=\'gis_data[save]\']', (event) => {
  ...
  /**
     ...
     */
  $(document).on('submit', '#gis_editor', (event) => {
    event.preventDefault();
    insertDataAndClose();
  });
  ...
  /**
     ...
     */
  $(document).on('change', '#gis_editor input[type=\'text\']', () => {
    const $form = $('form#gis_data_editor_form');
    const argsep = PMA_commonParams.get('arg_separator');
    $.post('gis_data_editor.php', `${$form.serialize() + argsep}generate=true${argsep}ajax_request=true`, (data) => {
      if (typeof data !== 'undefined' && data.success === true) {
  ...
  /**
     ...
     */
  $(document).on('change', '#gis_editor select.gis_type', (event) => {
    const $gis_editor = $('#gis_editor');
    const $form = $('form#gis_data_editor_form');

    const argsep = PMA_commonParams.get('arg_separator');
    $.post('gis_data_editor.php', `${$form.serialize() + argsep}get_gis_editor=true${argsep}ajax_request=true`, (data) => {
      if (typeof data !== 'undefined' && data.success === true) {
        $gis_editor.html(data.gis_editor);
        initGISEditorVisualization();
        prepareJSVersion();
  ...
  /**
     ...
     */
  $(document).on('click', '#gis_editor a.close_gis_editor, #gis_editor a.cancel_gis_editor', () => {
    closeGISEditor();
  });
  ...
  $(document).on('click', '#gis_editor a.addJs.addPoint', function () {
    ...
    const prefix = name.substr(0, name.length - 11);
    // Find the number of points
    const $noOfPointsInput = $(`input[name='${prefix}[no_of_points]` + '\']');
    const noOfPoints = parseInt($noOfPointsInput.val(), 10);
  ...
  $(document).on('click', '#gis_editor a.addLine.addJs', function () {
    ...
    const type = prefix.slice(prefix.lastIndexOf('[') + 1, prefix.lastIndexOf(']'));

    // Find the number of lines
    const $noOfLinesInput = $(`input[name='${prefix}[no_of_lines]` + '\']');
    const noOfLines = parseInt($noOfLinesInput.val(), 10);
  ...
  $(document).on('click', '#gis_editor a.addJs.addPolygon', function () {
    ...
    const prefix = name.substr(0, name.length - 13);
    // Find the number of polygons
    const $noOfPolygonsInput = $(`input[name='${prefix}[no_of_polygons]` + '\']');
    const noOfPolygons = parseInt($noOfPolygonsInput.val(), 10);

    // Add the new polygon
    let html = `${PMA_messages.strPolygon} ${noOfPolygons + 1}:<br/>`;
