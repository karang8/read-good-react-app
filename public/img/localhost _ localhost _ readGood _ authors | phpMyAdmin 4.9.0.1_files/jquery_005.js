...
// add the jquery instance method
  $.fn.drag = function (str, arg, opts) {
    ...
    const fn = $.isFunction(str) ? str : $.isFunction(arg) ? arg : null;
    // fix the event type
  ...
  var drag = $special.drag = {
    ...
    add(obj) {
      ...
      const data = $.data(this, drag.datakey);
    ...
    setup() {
      ...
      const data = $.extend({ related: 0 }, drag.defaults);
    ...
    interaction(elem, dd) {
      const offset = $(elem)[dd.relative ? 'position' : 'offset']() || { top: 0, left: 0 };
      return {
        drag: elem,
    ...
    hijack(event, type, dd, x, elem) {
      ...
      const orig = { event: event.originalEvent, type: event.type };
      // is the event drag related or drog related?
      const mode = type.indexOf('drop') ? 'drag' : 'drop';
      // iteration vars
      let result; let i = x || 0; let ia; let $elems; let callback;
      const len = !isNaN(x) ? x : dd.interactions.length;
    ...
    flatten(arr) {
      return $.map(arr, member => (member && member.jquery ? $.makeArray(member)
  ...
  var touchHooks = $event.fixHooks.touchstart = $event.fixHooks.touchmove = $event.fixHooks.touchend = $event.fixHooks.touchcancel = {
    ...
    filter(event, orig) {
      if (orig) {
        const touched = (orig.touches && orig.touches[0])
				|| (orig.changedTouches && orig.changedTouches[0])
				|| null;
        // iOS webkit: touchstart, touchmove, touchend
        if (touched) {
          $.each(touchHooks.props, (i, prop) => {
