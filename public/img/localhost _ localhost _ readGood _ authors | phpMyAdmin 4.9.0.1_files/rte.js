...
RTE.COMMON = {
  ...
  exportDialog($this) {
    ...
    if ($this.hasClass('mult_submit')) {
      ...
      const export_anchors = $('input.checkall:checked').parents('tr').find('.export_anchor');
      const count = export_anchors.length;
      let returnCount = 0;
    ...
    function showExport(data) {
      if (data.success === true) {
        ...
        const $ajaxDialog = $(`<div>${data.message}</div>`).dialog({
          width: 500,
        ...
        /**
                 ...
                 */
        const $elm = $ajaxDialog.find('textarea');
  ...
  dropDialog($this) {
    /**
         ...
         */
    const $curr_row = $this.parents('tr');
    /**
         * @var question String containing the question to be asked for confirmation
         */
    const question = $('<div/>').text(
  ...
  dropMultipleDialog($this) {
    // We ask for confirmation here
    $this.PMA_confirm(PMA_messages.strDropRTEitems, '', (url) => {
      /**
             * @var msg jQuery object containing the reference to
             *          the AJAX message shown to the user
             */
      ...
      const drop_anchors = $('input.checkall:checked').parents('tr').find('.drop_anchor');
      let success = true;
      const count = drop_anchors.length;
      let returnCount = 0;
      ...
      drop_anchors.each(function () {
        ...
        $.post($anchor.attr('href'), params, (data) => {
          returnCount++;
          if (data.success === true) {
            /**
                         * @var $table Object containing reference
...
RTE.ROUTINE = {
  ...
  postDialogShow(data) {
    ...
    $('table.routine_params_table').last().find('tr').has('td')
      .each(function () {
  ...
  validateCustom() {
    ...
    if (this.$ajaxDialog.find('select[name=item_type]').find(':selected').val() === 'FUNCTION') {
      ...
      const $returntyp = this.$ajaxDialog.find('select[name=item_returntype]');
      const $returnlen = this.$ajaxDialog.find('input[name=item_returnlength]');
    ...
    if ($('select[name=item_type]').find(':selected').val() === 'FUNCTION') {
      ...
      if (this.$ajaxDialog.find('table.rte_table').find('textarea[name=item_definition]').val().toUpperCase()
        .indexOf('RETURN') < 0) {
        this.syntaxHiglighter.focus();
      ...
      if (data.success === true) {
        ...
        if (data.dialog) {
          ...
          that.buttonOptions[PMA_messages.strGo] = function () {
            ...
            $.post('db_routines.php', data, (data) => {
              if (data.success === true) {
...
$(() => {
  ...
  $(document).on('click', 'a.ajax.add_anchor, a.ajax.edit_anchor', function (event) {
    event.preventDefault();
    let type = $(this).attr('href').substr(0, $(this).attr('href').indexOf('?'));
    ...
    const dialog = new RTE.object(type);
  ...
  $(document).on('change', 'select[name^=item_param_type]', function () {
    ...
    const rte = new RTE.object('routine');
  ...
  $(document).on('click', 'input[name=routine_addparameter]', function (event) {
    ...
    /**
         ...
         */
    const $routine_params_table = $(this).closest('div.ui-dialog').find('.routine_params_table');
    /**
         * @var new_param_row A string containing the HTML code for the
         *                    new row for the routine parameters table
         */
    const new_param_row = RTE.param_template.replace(/%s/g, $routine_params_table.find('tr').length - 1);
    ...
    /**
         ...
         */
    const $newrow = $(this).closest('div.ui-dialog').find('table.routine_params_table').find('tr')
      .has('td')
      .last();
    // Enable/disable the 'options' dropdowns for parameters as necessary
    const rte = new RTE.object('routine');
