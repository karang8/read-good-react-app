 ...
 */
AJAX.registerTeardown('config.js', () => {
...
AJAX.registerOnload('config.js', () => {
  const $topmenu_upt = $('#topmenu2.user_prefs_tabs');
...
function getAllValues() {
  const $elements = $('fieldset input, fieldset select, fieldset textarea');
  const values = {};
  let type;
  let value;
...
var validators = {
  ...
  /**
     ...
     */
  PMA_validatePositiveNumber(isKeyUp) {
    if (isKeyUp && this.value === '') {
      return true;
    }
    const result = this.value !== '0' && validators._regexp_numeric.test(this.value);
  ...
  /**
     ...
     * @param {boolean} isKeyUp
     */
  PMA_validateNonNegativeNumber(isKeyUp) {
    if (isKeyUp && this.value === '') {
      return true;
    }
    const result = validators._regexp_numeric.test(this.value);
  ...
  /**
     ...
     * @param {boolean} isKeyUp
     */
  PMA_validatePortNumber(isKeyUp) {
    if (this.value === '') {
      return true;
    }
    const result = validators._regexp_numeric.test(this.value) && this.value !== '0';
  ...
  /**
     ...
     * @param {string}  regexp
     */
  PMA_validateByRegex(isKeyUp, regexp) {
    if (isKeyUp && this.value === '') {
      return true;
    }
    // convert PCRE regexp
    const parts = regexp.match(validators._regexp_pcre_extract);
  ...
  /**
     ...
     * @param {int} max_value
     */
  PMA_validateUpperBound(isKeyUp, max_value) {
...
function getFieldValidators(field_id, onKeyUpOnly) {
  ...
  const name = field_id && field_id.match(/[^-]+$/)[0];
  ...
  if (typeof validate[field_id] !== 'undefined') {
    ...
    for (let i = 0, imax = validate[field_id].length; i < imax; i++) {
...
function displayErrors(error_list) {
  ...
  for (const field_id in error_list) {
    let errors = error_list[field_id];
    ...
    if (errors.length) {
      ...
      let html = '';
      for (let i = 0, imax = errors.length; i < imax; i++) {
        html += `<dd>${errors[i]}</dd>`;
      }
      $errorCnt.html(html);
...
function validate_fieldset(fieldset, isKeyUp, errors) {
  ...
  if ($fieldset.length && typeof validators._fieldset[$fieldset.attr('id')] !== 'undefined') {
    const fieldset_errors = validators._fieldset[$fieldset.attr('id')].apply($fieldset[0], [isKeyUp]);
    for (const field_id in fieldset_errors) {
...
function validate_field(field, isKeyUp, errors) {
  ...
  const field_id = $field.attr('id');
  errors[field_id] = [];
  const functions = getFieldValidators(field_id, isKeyUp);
...
function setupValidation() {
  ...
  // form values
  const $check_page_refresh = $('#check_page_refresh');
...
function setTab(tab_id) {
  $('ul.tabs').each(function () {
    ...
    $this.find('li').removeClass('active').find(`a[href="#${tab_id}"]`).parent()
      .addClass('active');
    $this.parent().find('div.tabs_contents fieldset').hide().filter(`#${tab_id}`)
      .show();
    const hashValue = `tab_${tab_id}`;
...
function setupConfigTabs() {
  const forms = $('form.config-form');
  forms.each(function () {
    const $this = $(this);
    const $tabs = $this.find('ul.tabs');
    if (!$tabs.length) {
      return;
    }
...
AJAX.registerOnload('config.js', () => {
  ...
  const ls_supported = isStorageSupported('localStorage', true);
  const ls_exists = ls_supported ? (window.localStorage.config || false) : false;
...
function updatePrefsDate() {
  const d = new Date(window.localStorage.config_mtime_local);
  const msg = PMA_messages.strSavedOn.replace(
    '@DATE@',
...
function offerPrefsAutoimport() {
  const has_config = (isStorageSupported('localStorage')) && (window.localStorage.config || false);
