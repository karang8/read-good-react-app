...
function onKeyDownArrowsHandler(e) {
  ...
  if (is_firefox) {
    ...
    for (ffversion = 3; ffversion < 25; ffversion++) {
      const is_firefox_v_24 = navigator.userAgent.toLowerCase().indexOf(`firefox/${ffversion}`) > -1;
      if (is_firefox_v_24) {
        ffcheck = 1;
        break;
      }
...
AJAX.registerOnload('keyhandler.js', () => {
  $(document).on('keydown keyup', '#table_columns', (event) => {
    onKeyDownArrowsHandler(event.originalEvent);
  });
  $(document).on('keydown keyup', 'table.insertRowTable', (event) => {
    onKeyDownArrowsHandler(event.originalEvent);
  });
});
