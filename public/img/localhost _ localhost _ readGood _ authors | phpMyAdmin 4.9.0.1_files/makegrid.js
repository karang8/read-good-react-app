...
function PMA_makegrid(t, enableResize, enableReorder, enableVisib, enableGridEdit) {
  var g = {
    ...
    reposRsz() {
      ...
      const $firstRowCols = $(g.t).find('tr:first th.draggable:visible');
      ...
      for (let n = 0, l = $firstRowCols.length; n < l; n++) {
    ...
    showEditCell(cell) {
      if ($(cell).is('.grid_edit')
        ...
        if (!g.isCellEditActive) {
          ...
          if ($cell.attr('data-type') === 'string'
                        || $cell.attr('data-type') === 'blob'
          ...
          let value = PMA_getCellValue(cell);
    ...
    /**
         ...
         */
    hideEditCell(force, data, field, options) {
      ...
      if (data) {
        if (g.currentEditCell) { // save value of currently edited cell
          ...
          const $this_field = $(g.currentEditCell);
          const is_null = $this_field.data('value') === null;
          if (is_null) {
            ...
            const $target = $this_field.find(selector);
        ...
        if (data.transformations !== undefined) {
          $.each(data.transformations, (cell_index, value) => {
            const $this_field = $(g.t).find(`.to_be_saved:eq(${cell_index})`);
            $this_field.find('span').html(value);
          });
        }
        if (data.relations !== undefined) {
          $.each(data.relations, (cell_index, value) => {
            const $this_field = $(g.t).find(`.to_be_saved:eq(${cell_index})`);
            $this_field.find('span').html(value);
          });
        }
    ...
    showEditArea() {
      if (!g.isCellEditActive) { // make sure the edit area has not been shown
        ...
        /**
                 ...
                 */
        const where_clause = $td.parent('tr').find('.where_clause').val();
        /**
                 * @var field_name  String containing the name of this field.
                 * @see getFieldName()
                 */
        const field_name = getFieldName($(t), $td);
        /**
                 * @var relation_curr_value String current value of the field (for fields that are foreign keyed).
                 */
        const relation_curr_value = $td.text();
        /**
        ...
        if ($td.is(':not(.not_null)')) {
          ...
          if ($td.is('.enum, .set')) {
            ...
            $editArea.on('click', '.browse_foreign', () => {
              $checkbox.prop('checked', false);
            });
          } else {
        ...
        if ($td.is('.relation')) {
          ...
          g.lastXHR = $.post('sql.php', post_params, (data) => {
            g.lastXHR = null;
          ...
          if ($td.is('.to_be_saved')) { // cell has been edited
            ...
            /**
                         ...
                         */
            const sql_query = `SELECT \`${field_name}\` FROM \`${g.table}\` WHERE ${where_clause}`;
          ...
          const $input_field = $(g.cEdit).find('.edit_box');
    ...
    postEditedCell(options) {
      ...
      /**
             ...
             */
      const relational_display = $(g.o).find('input[name=relational_display]:checked').val();
      /**
      ...
      /**
             ...
             */
      let transform_fields_list = '';
      /**
             * @var where_clause Array containing where clause for updated fields
             */
      const full_where_clause = [];
      /**
             * @var is_unique   Boolean, whether the rows in this table is unique or not
             */
      const is_unique = $(g.t).find('td.edit_row_anchor').is('.nonunique') ? 0 : 1;
      /**
             * multi edit variables
             */
      const me_fields_name = [];
      const me_fields_type = [];
      const me_fields = [];
      const me_fields_null = [];
      ...
      $(g.t).find('td.to_be_saved').parents('tr').each(function () {
        const $tr = $(this);
        let where_clause = $tr.find('.where_clause').val();
        if (typeof where_clause === 'undefined') {
          where_clause = '';
        }
        full_where_clause.push(where_clause);
        const condition_array = JSON.parse($tr.find('.condition_array').val());
        ...
        $tr.find('.to_be_saved').each(function () {
          ...
          /**
                     ...
                     */
          const is_null = this_field_params[field_name] === null;
          ...
          if (is_null) {
            ...
            const cell_index = $this_field.index('.to_be_saved');
          ...
          if (where_clause.indexOf(PMA_urlencode(field_name)) > -1) {
            const field_str = `\`${g.table}\`.` + `\`${field_name}\``;
            for (const field in condition_array) {
      ...
      $.ajax({
        ...
        success(data) {
          ...
          if (typeof data !== 'undefined' && data.success === true) {
            ...
            $(g.t).find('td.to_be_saved').parents('tr').each(function () {
              const new_clause = $(this).data('new_clause');
              const $where_clause = $(this).find('.where_clause');
              const old_clause = $where_clause.val();
              const decoded_old_clause = old_clause;
              const decoded_new_clause = new_clause;
              ...
              $(this).find('input[type=checkbox]').each(function () {
                const $checkbox = $(this);
                const checkbox_name = $checkbox.attr('name');
                const checkbox_value = $checkbox.val();
            ...
            if (typeof data.sql_query !== 'undefined') {
              // extract query box
              const $result_query = $($.parseHTML(data.sql_query));
              const sqlOuter = $result_query.find('.sqlOuter').wrap('<p>').parent().html();
              const tools = $result_query.find('.tools').wrap('<p>').parent().html();
              ...
              if (typeof sqlOuter !== 'undefined' && typeof tools !== 'undefined') {
                ...
                const $existing_query = $(g.o).find('.result_query');
    ...
    saveEditedCell() {
      /**
             ...
             */
      const $this_field = $(g.currentEditCell);
      let $test_element = ''; // to test the presence of a element

      let need_to_post = false;
      ...
      /**
             * @var is_null String capturing whether 'checkbox_null_<field_name>_<row_index>' is checked.
             */
      const is_null = $(g.cEdit).find('input:checkbox').is(':checked');
      ...
      if ($(g.cEdit).find('.edit_area').is('.edit_area_loading')) {
        ...
        if ($this_field.is('.bit')) {
          ...
          if ($(g.cEdit).find('.edit_box').val().match(/^(0x)?[a-f0-9]*$/i) !== null) {
            ...
            const hexError = `<div class="error">${PMA_messages.strEnterValidHex}</div>`;
    ...
    initColResize() {
      ...
      const $firstRowCols = $(g.t).find('tr:first th.draggable');
    ...
    initColReorder() {
      ...
      $(g.o).find('div.restore_column').click(() => {
        g.restoreColOrder();
      });
      ...
      $(g.t).find('th a').on('dragstart', () => false);
    ...
    initColVisib() {
      ...
      const $col_visib = $(g.o).find('.col_visib'); // check if column visibility is passed from PHP
      ...
      if ($firstRowCols.length > 1) {
        const $colVisibTh = $(g.t).find('th:not(.draggable)');
        PMA_tooltip(
          $colVisibTh,
          'th',
      ...
      $(g.t).find('td, th.draggable').mouseenter(() => {
        g.hideColList();
      });
    ...
    moveUp(e) {
      ...
      let where_clause = $this_field.parents('tr').first().find('.where_clause').val();
      if (typeof where_clause === 'undefined') {
        where_clause = '';
      }
      let found = false;
      ...
      $this_field.parents('tr').first().parents('tbody').children()
        .each(function () {
    ...
    initGridEdit() {
      ...
      $(g.cEditStd).find('.edit_box').focus(() => {
        g.showEditArea();
      });
      $(g.cEditStd).on('keydown', '.edit_box, select', (e) => {
        if (e.which === 13) {
          // post on pressing "Enter"
          e.preventDefault();
          g.saveOrPostEditedCell();
      ...
      $(g.cEditTextarea).find('.edit_box').focus(() => {
        g.showEditArea();
      });
      $(g.cEditTextarea).on('keydown', '.edit_box, select', (e) => {
      ...
      $(g.o).find('div.save_edited').click(() => {
        g.hideEditCell();
        g.postEditedCell();
      });
  ...
  // get data columns in the first row of the table
  const $firstRowCols = $(t).find('tr:first th.draggable');
  ...
  const $common_hidden_inputs = $(g.o).find('div.common_hidden_inputs');
...
(function ($) {
  $.fn.noSelect = function (p) { // no select plugin by Paulo P.Marinas
    ...
    const is_msie = navigator.userAgent.indexOf('MSIE') > -1 || !!window.navigator.userAgent.match(/Trident.*rv\:11\./);
