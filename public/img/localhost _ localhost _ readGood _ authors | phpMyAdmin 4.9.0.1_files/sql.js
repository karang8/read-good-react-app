...
function getFieldName($table_results, $this_field) {
  const this_field_index = $this_field.index();
  // ltr or rtl direction does not impact how the DOM was generated
  // check if the action column in the left exist
  const left_action_exist = !$table_results.find('th:first').hasClass('draggable');
  // number of column span for checkbox and Actions
  const left_action_skip = left_action_exist ? $table_results.find('th:first').attr('colspan') - 1 : 0;
  ...
  if (field_name === '') {
    const $heading = $table_results.find('thead').find(`th:eq(${this_field_index - left_action_skip})`).children('span');
...
AJAX.registerOnload('sql.js', () => {
  $(() => {
    if (codemirror_editor) {
      ...
      $('#sqlquery').on('input propertychange', () => {
        PMA_autosaveSQL($('#sqlquery').val());
      });
      // Save sql query with sort
      if ($('#RememberSorting') !== undefined && $('#RememberSorting').is(':checked')) {
        $('select[name="sql_query"]').on('change', () => {
          PMA_autosaveSQLSort($('select[name="sql_query"]').val());
        });
      } else if (isStorageSupported('localStorage') && window.localStorage.auto_saved_sql_sort !== undefined) {
      ...
      const sortStoredQuery = (isStorageSupported('localStorage') && typeof window.localStorage.auto_saved_sql_sort !== 'undefined') ? window.localStorage.auto_saved_sql_sort : Cookies.get('auto_saved_sql_sort');
  ...
  $(document).on('click', 'a.delete_row.ajax', function (e) {
    e.preventDefault();
    const question = PMA_sprintf(PMA_messages.strDoYouReally, escapeHtml($(this).closest('td').find('div').text()));
    const $link = $(this);
    $link.PMA_confirm(question, $link.attr('href'), (url) => {
  ...
  $(document).on('submit', '.bookmarkQueryForm', function (e) {
    ...
    PMA_ajaxShowMessage();
    const argsep = PMA_commonParams.get('arg_separator');
    $.post($(this).attr('action'), `ajax_request=1${argsep}${$(this).serialize()}`, (data) => {
      if (data.success) {
        PMA_ajaxShowMessage(data.message);
      } else {
        PMA_ajaxShowMessage(data.error, false);
  ...
  /**
     ...
     */
  $(document).on('click', '#copyToClipBoard', (event) => {
    ...
    $('.table_results .column_heading a').each(function () {
      ...
      textArea.value += `${$(this).clone().find('small').remove()
        .end()
        .text()}\t`;
    });
  ...
  /**
     ...
     * @memberOf    jQuery
     */
  $(document).on('makegrid', '.sqlqueryresults', () => {
  ...
  /*
     ...
     * @memberOf    jQuery
     */
  $(document).on('stickycolumns', '.sqlqueryresults', () => {
    $('.sticky_columns').remove();
    $('.table_results').each(function () {
      const $table_results = $(this);
      // add sticky columns div
      const $stick_columns = initStickyColumns($table_results);
  ...
  $(document).on('change', '#id_bookmark', function (event) {
    ...
    const $varDiv = $('#bookmark_variables');
    $varDiv.empty();
  ...
  /**
     ...
     * @memberOf    jQuery
     */
  $('input[name=bookmark_variable]').on('keypress', (event) => {
  ...
  $(document).on('submit', '#sqlqueryform.ajax', function (event) {
    ...
    const argsep = PMA_commonParams.get('arg_separator');
    $.post($form.attr('action'), `${$form.serialize() + argsep}ajax_page_request=true`, (data) => {
      if (typeof data !== 'undefined' && data.success === true) {
        ...
        if (typeof data.ajax_reload !== 'undefined') {
          ...
          PMA_commonActions.refreshMain(url, () => {
            $('#sqlqueryresultsouter')
              .show()
              .html(data.message);
            PMA_highlightSQL($('#sqlqueryresultsouter'));
  ...
  $(document).on('submit', 'form[name=\'displayOptionsForm\'].ajax', function (event) {
    ...
    const $msgbox = PMA_ajaxShowMessage();
    const argsep = PMA_commonParams.get('arg_separator');
    $.post($form.attr('action'), `${$form.serialize() + argsep}ajax_request=true`, (data) => {
      PMA_ajaxRemoveMessage($msgbox);
  ...
  $(document).on('keyup', '.filter_rows', function () {
    const unique_id = $(this).data('for');
    const $target_table = $(`.table_results[data-uniqueId='${unique_id}']`);
    const $header_cells = $target_table.find('th[data-column]');
    const target_columns = Array();
    // To handle colspan=4, in case of edit,copy etc options.
    const dummy_th = ($('.edit_row_anchor').length !== 0
  ...
  $('body').on('click', '.navigation .showAllRows', function (e) {
    ...
    if (!$(this).is(':checked')) { // already showing all rows
      ...
      $form.PMA_confirm(PMA_messages.strShowAllRowsWarning, $form.attr('action'), (url) => {
        submitShowAllForm();
      });
    }
  ...
  /**
     ...
     */
  $('body').on('click', '#simulate_dml', () => {
    const $form = $('#sqlqueryform');
    let query = '';
    const delimiter = $('#id_sql_delimiter').val();
    const db_name = $form.find('input[name="db"]').val();
    ...
    $.ajax({
      ...
      success(response) {
        PMA_ajaxRemoveMessage($msgbox);
        if (response.success) {
          let dialog_content = '<div class="preview_sql">';
          ...
          const $response_dialog = $('<div />').append($dialog_content).dialog({
            minWidth: 540,
            maxHeight: 400,
            modal: true,
...
function PMA_changeClassForColumn($this_th, newclass, isAddClass) {
  ...
  let th_index = $this_th.index();
  const has_big_t = $this_th.closest('tr').children(':first').hasClass('column_action');
  // .eq() is zero-based
  if (has_big_t) {
    th_index--;
  }
  let $table = $this_th.parents('.table_results');
  if (!$table.length) {
    $table = $this_th.parents('table').siblings('.table_results');
  }
  const $tds = $table.find('tbody tr').find(`td.data:eq(${th_index})`);
...
function browseForeignDialog($this_a) {
  ...
  let $dialog = null;
  const argSep = PMA_commonParams.get('arg_separator');
  let params = $this_a.getPostData();
  params += `${argSep}ajax_request=true`;
  $.post($this_a.attr('href'), params, (data) => {
    ...
    $(tableId).on('click', 'td a.foreign_value', function (e) {
      e.preventDefault();
      let $input = $this_a.prev('input[type=text]');
    ...
    $(formId).on('submit', function (e) {
      ...
      $.post(`${$(this).attr('action')}?ajax_request=1`, postParams, (data) => {
...
function makeProfilingChart() {
  ...
  const data = [];
  $.each(JSON.parse($('#profilingChartData').html()), (key, value) => {
...
function setStickyColumnsPosition($sticky_columns, $table_results, position, top, left, margin_left) {
  $sticky_columns
    ...
    .css('top', top)
    .css('left', left || 'auto')
    .css('margin-left', margin_left || '0px')
...
function rearrangeStickyColumns($sticky_columns, $table_results) {
  const $originalHeader = $table_results.find('thead');
  const $originalColumns = $originalHeader.find('tr:first').children();
  const $clonedHeader = $originalHeader.clone();
  // clone width per cell
  $clonedHeader.find('tr:first').children().width((i, val) => {
    let width = $originalColumns.eq(i).width();
...
function handleStickyColumns($sticky_columns, $table_results) {
  ...
  const tableStartOffset = $table_results.offset().top;
  const tableEndOffset = tableStartOffset + $table_results.height();
